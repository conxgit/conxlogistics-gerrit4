...
public class TaskWizard extends Wizard implements ITaskWizard, IPageFlowPageChangedEventHandler {
	...
	private void addAllPages() {
		if (initParams == null) {
			...
			initParams.put(IPageComponent.ENTITY_CONTAINER_PROVIDER, new IEntityContainerProvider() {
		...
		if (session.getPages() != null) {
			for (IPageFlowPage page : session.getPages()) {
				IPageComponent pageComponent = this.pageFactory.create((IPageFlowPage) page, initParams);
	...
	private void completeCurrentTaskAndAdvanceToNext() throws Exception {
		...
		if (currentPage.isExecuted()) {
			engine.updateProcessInstanceVariables(this, currentPage.getResultData());
		} else {
		...
		if (session.getPages() != null) {
			...
			for (IPageFlowPage page : session.getPages()) {
				...
				if (!stepAlreadyExists) {
					pageComponent = this.pageFactory.create((IPageFlowPage) page, initParams);
					this.pageComponentMap.put(page, pageComponent);
					addStep(pageComponent);
				}
			}
