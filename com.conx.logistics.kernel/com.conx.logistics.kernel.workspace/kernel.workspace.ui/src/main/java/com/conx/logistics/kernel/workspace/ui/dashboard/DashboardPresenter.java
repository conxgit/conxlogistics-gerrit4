...
public class DashboardPresenter extends
		FactoryPresenter<IDashboardView, DashboardEventBus> {
	...
	public void onLaunch(MainMVPApplication mainApp) {
		if (!isLaunched)
		{
			...
			//-- Configure Tasks
			this.humanTaskEM = mainApp.getConxHumanTaskEntityManagerFactory()
					.createEntityManager();
			
/*			this.taskContainer = JPAContainerFactory.make(TaskSummary.class,this.humanTaskEM);
 			this.taskContainer = JPAContainerFactory.make(Task.class,this.humanTaskEM);
			this.taskContainer.addNestedContainerProperty("taskData.actualOwner.id");
			this.taskContainer.addNestedContainerProperty("taskData.createdOn");
			this.taskContainer.addNestedContainerProperty("taskData.processId");
			this.taskContainer.addNestedContainerProperty("taskData.processInstanceId");
			...
			((DashboardView)getView()).getMyTasksTable().getFilterTable().setColumnHeader("taskData.createdOn", "Created");
			((DashboardView)getView()).getMyTasksTable().getFilterTable().setColumnHeader("taskData.actualOwner.id", "Owner");
			((DashboardView)getView()).getMyTasksTable().getFilterTable().setColumnHeader("taskData.processId", "Process");
			((DashboardView)getView()).getMyTasksTable().getFilterTable().setColumnHeader("taskData.processInstanceId", "Process Instance ID");
			
			((DashboardView)getView()).getMyTasksTable().getFilterTable().setVisibleColumns(new String[]{"id","taskData.createdOn","taskData.actualOwner.id","taskData.processId","taskData.processInstanceId"});
*/		
			
			
			this.taskContainer = new JPAContainer<TaskSummary> (TaskSummary.class);
			NamedQueryLocalEntityProvider namedQueryProvider = new NamedQueryLocalEntityProvider(TaskSummary.class,"TasksOwnedWithNulls",this.humanTaskEM);
			...
			((DashboardView)getView()).getMyTasksTable().getFilterTable().setVisibleColumns(new String[]{"id","name","status","comment","createdOn","actualOwner.id","processId","processInstanceId"});

			
			this.isLaunched= true; 
			
/*			this.taskContainer.getEntityProvider().setQueryModifierDelegate(new DefaultQueryModifierDelegate() {
				...
				public void filtersWillBeAdded(CriteriaBuilder criteriaBuilder, CriteriaQuery<?> query, List<Predicate> predicates) {
					...
					Path<String> taskOwnerUserId = taskRoot.<org.jbpm.task.User> get("actualOwner").get("id");
					predicates.add(criteriaBuilder.equal(taskOwnerUserId, DashboardPresenter.this.currentUser.getScreenName()));
				}
			});*/
			...
			((DashboardView)getView()).getMyTasksTable().getFilterTable().addListener(new ItemClickListener() {
				...
				public void itemClick(ItemClickEvent event) {
					if (event.isDoubleClick())
					{
						...
						Feature taskFeature = DashboardPresenter.this.mainApp.getFeatureDAOService().findFeatureByCode(ts.getProcessId());
						taskFeature.setTaskId(Long.toString(ts.getId()));
						DashboardPresenter.this.mainApp.getMainEventBus().onOpenApplicationFeature(taskFeature);
					}
				}
			});
		}
