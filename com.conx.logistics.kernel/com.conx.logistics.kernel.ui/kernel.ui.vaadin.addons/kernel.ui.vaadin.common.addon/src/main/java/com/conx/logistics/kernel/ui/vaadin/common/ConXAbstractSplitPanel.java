...
public abstract class ConXAbstractSplitPanel extends AbstractLayout {
	...
	private Set<ISplitPositionChangeListener> splitPositionChangeListeners = new HashSet<ConXAbstractSplitPanel.ISplitPositionChangeListener>();

	private int firstComponentHeight = -1;

	private int secondComponentHeight = -1;
	 ...
	 *            knows the position.
	 */
	private void setSplitPosition(int pos, int unit, boolean repaintNeeded, boolean reverse) {
		...
		onSplitPositionChanged(pos, posUnit, posReversed);
	}
	...
	public void addSplitPositionChangeListener(ISplitPositionChangeListener listener) {
		splitPositionChangeListeners.add(listener);
	}

	private void onSplitPositionChanged(Integer newPos, int posUnit, boolean posReversed) {
		for (ISplitPositionChangeListener listener : splitPositionChangeListeners) {
			listener.onSplitPositionChanged(newPos, posUnit, posReversed);
		}
	}

	private void onFirstComponentHeightChanged(int height) {
		if (height != this.firstComponentHeight) {
			for (ISplitPositionChangeListener listener : splitPositionChangeListeners) {
				listener.onFirstComponentHeightChanged(height);
			}
			this.firstComponentHeight = height;
		}
	}

	private void onSecondComponentHeightChanged(int height) {
		if (height != this.secondComponentHeight) {
			for (ISplitPositionChangeListener listener : splitPositionChangeListeners) {
				listener.onSecondComponentHeightChanged(height);
			}
	...
	public void changeVariables(Object source, Map<String, Object> variables) {
		...
		if (variables.containsKey("firstContainerHeight") && !isLocked()) {
			Integer height = (Integer) variables.get("firstContainerHeight");
//			variables.remove("firstContainerHeight");
			onFirstComponentHeightChanged(height);
		}
		if (variables.containsKey("secondContainerHeight") && !isLocked()) {
			Integer height = (Integer) variables.get("secondContainerHeight");
//			variables.remove("secondContainerHeight");
			onSecondComponentHeightChanged(height);
		}
	...
	public interface ISplitPositionChangeListener {
