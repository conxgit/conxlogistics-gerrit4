...
import com.conx.logistics.mdm.domain.metamodel.ListAttribute;
import com.conx.logistics.mdm.domain.metamodel.MapAttribute;
import com.conx.logistics.mdm.domain.metamodel.SetAttribute;
...
public class EntityTypeDAOImpl implements IEntityTypeDAOService {
	...
	public List<EntityType> getAll() {
		return em.createQuery("select o from com.conx.logistics.mdm.domain.metamodel.metadata.EntityType o record by o.id", EntityType.class).getResultList();
	}

	@Override
	public List<AbstractAttribute> getAllAttributesByEntityType(EntityType parentEntityType) {
		TypedQuery<AbstractAttribute> q = em.createQuery("select o from com.conx.logistics.mdm.domain.metamodel.AbstractAttribute o WHERE o.parentEntityType = :parentEntityType",
	...
	@Override
	public List<com.conx.logistics.mdm.domain.metamodel.SingularAttribute> getAllSingularAttributesByEntityType(EntityType parentEntityType) {
		TypedQuery<com.conx.logistics.mdm.domain.metamodel.SingularAttribute> q = em.createQuery(
				"select o from com.conx.logistics.mdm.domain.metamodel.SingularAttribute o WHERE o.parentEntityType = :parentEntityType",
				com.conx.logistics.mdm.domain.metamodel.SingularAttribute.class);
		q.setParameter("parentEntityType", parentEntityType);
		return q.getResultList();
	}

	@Override
	public List<com.conx.logistics.mdm.domain.metamodel.PluralAttribute> getAllPluralAttributesByEntityType(EntityType parentEntityType) {
		TypedQuery<com.conx.logistics.mdm.domain.metamodel.PluralAttribute> q = em.createQuery(
				"select o from com.conx.logistics.mdm.domain.metamodel.PluralAttribute o WHERE o.parentEntityType = :parentEntityType", com.conx.logistics.mdm.domain.metamodel.PluralAttribute.class);
		q.setParameter("parentEntityType", parentEntityType);
		return q.getResultList();
	}
	...
	public List<BasicAttribute> getAllBasicAttributesByEntityType(EntityType parentEntityType) {
		TypedQuery<BasicAttribute> q = em.createQuery("select o from com.conx.logistics.mdm.domain.metamodel.BasicAttribute o WHERE o.parentEntityType = :parentEntityType", BasicAttribute.class);
		q.setParameter("parentEntityType", parentEntityType);
	...
	public Long countByClass(Class entityClass) throws Exception {
		...
		try {
			 ...
			 */
			Query q = em.createQuery("select COUNT(o) from com.conx.logistics.mdm.domain.metamodel.EntityType o WHERE o.entityJavaSimpleType = :entityJavaSimpleType", Long.class);
	...
	public EntityType getByClass(Class entityClass) throws Exception {
		...
		try {
			 ...
			 * return typedQuery.getSingleResult();
			 */
			typedQuery = em.createQuery("select DISTINCT  o from com.conx.logistics.mdm.domain.metamodel.EntityType o WHERE o.entityJavaSimpleType = :entityJavaSimpleType", EntityType.class);
	...
	public EntityType provide(IdentifiableType jpaEntityType) throws Exception {
		...
		if (count == 0) {
			...
			try {
				...
				e.printStackTrace(new PrintWriter(sw));
				String stacktrace = sw.toString();
				logger.info("Entity (" + jpaEntityType.getJavaType() + ") is already a MappedSuperType.");
			...
			if (mt != null) {
				if (mt instanceof javax.persistence.metamodel.EntityType) {
					targetEntityType = new EntityType(((javax.persistence.metamodel.EntityType) mt).getName(), mt.getJavaType(), st, null,// SingularAttribute
																																			// id,
					...
					targetEntityType = new EntityType(((EntityType) mt).getName(), mt.getJavaType(), null, null,// SingularAttribute
																												// id,
							null,// SingularAttribute version,
							mt.getJavaType().getName());
					targetEntityType = add(targetEntityType);
				...
				if (targetEntityType == null) {
					...
					targetEntityType = new EntityType(jpaEntityType.getJavaType().getName(), jpaEntityType.getJavaType(), st, null,// SingularAttribute
																																	// id,
							null,// SingularAttribute version,
	...
	private EntityType updateEntityTypeAttributes(EntityType targetEntityType, EntityType superTargetEntityType, IdentifiableType jpaEntityType) throws Exception {
		// Process attributes
		...
		EntityType et;
		com.conx.logistics.mdm.domain.metamodel.AbstractAttribute attr;
		EntityType attrEntityType;
		for (SingularAttribute sattr : sattrs) {
			attr = null;
			at = sattr.getPersistentAttributeType();
			if (at == PersistentAttributeType.BASIC) {
				attr = new com.conx.logistics.mdm.domain.metamodel.BasicAttribute(sattr.isId(), sattr.getName(), sattr.getJavaType(), targetEntityType);
				...
				attr = new com.conx.logistics.mdm.domain.metamodel.SingularAttribute(sattr.getName(), sattr.getJavaType(), sattr.isId(), sattr.isVersion(), sattr.isOptional(), at, attrEntityType);
				attr.setParentEntityType(targetEntityType);
				attr = add(attr);
				System.out.println("[" + entity + "] Adding ONE_TO_ONE/MANY_TO_ONE attr (" + attr.getName() + ")");
				targetEntityType.getDeclaredAttributes().add(new EntityTypeAttribute(targetEntityType, (com.conx.logistics.mdm.domain.metamodel.SingularAttribute) attr));
			}

		}

		com.conx.logistics.mdm.domain.metamodel.PluralAttribute pattr_;
		for (PluralAttribute pattr : pattrs) {
			pattr_ = null;
			...
			if (at == PersistentAttributeType.ONE_TO_MANY || at == PersistentAttributeType.MANY_TO_MANY) {
				...
				if (pattr.getCollectionType() == CollectionType.LIST) {
					...
					pattr_ = (com.conx.logistics.mdm.domain.metamodel.PluralAttribute) add(pattr_);
	...
	public EntityType createMappedSupperClass(IdentifiableType supertype) throws Exception {
		...
		if (record == null) {
			...
			record = new EntityType(supertype.getJavaType().getName(), supertype.getJavaType(), sst_, null,// SingularAttribute
																											// id,
					null,// SingularAttribute version,
