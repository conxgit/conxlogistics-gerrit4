...
class EventBusHandler implements InvocationHandler {
	...
	private CustomizedEventReceiverRegistry handlerRegistry;

	private String busName;

	public EventBusHandler(EventBusManager em, EventReceiverRegistry hr, String name) {
		this.eventManager = em;
		this.handlerRegistry = (CustomizedEventReceiverRegistry) hr;
		this.busName = name;
	}
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		for (Class<?> handlerType : handlerTypes) {
			Set<?> handlers = handlerRegistry.lookupReceivers(handlerType);
			if (handlers == null) {
				logger.info("Handlers for {} not registered", handlerType.getName());
				continue;
			}

			// invoke handlers; the actual method is the event name prefixed
			// with
			// "on..."
			...
			for (Object handler : handlers) {
				try {
					...
					logger.warn("{} defined as a receiver for event {} but no method {}" + " could be found with matching arguments", msgArgs);
				}
				try {
					...
					// CRAP!
					// MainEventBus mainBus =
					// eventManager.getEventBus(MainEventBus.class);
					// mainBus.error(t);
				}
			}
		}
		/*
		 * forwarding to parent disabled / there's no possibility to declare the
		 * parent
		 */
		/*
		 * if (eventDef.forwardToParent() && parent != null) { try { Method m =
		 * parent.getClass().getMethod(method.getName(),
		 * method.getParameterTypes()); m.invoke(parent, args); } catch
