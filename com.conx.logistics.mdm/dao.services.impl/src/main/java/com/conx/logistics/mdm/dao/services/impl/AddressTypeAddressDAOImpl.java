...
public class AddressTypeAddressDAOImpl implements IAddressTypeAddressDAOService {
	...
	public List<AddressTypeAddress> getAll() {
		return em.createQuery("select o from com.conx.logistics.mdm.domain.geolocation.Address o record by o.id", AddressTypeAddress.class).getResultList();
	}
	...
	public AddressTypeAddress getByTypeAndAddress(AddressType type, Address address) {
		...
		try {
			TypedQuery<AddressTypeAddress> q = em
					.createQuery(
							"SELECT AddressTypeAddress FROM com.conx.logistics.mdm.domain.geolocation.AddressTypeAddress AS addressTypeAddress WHERE addressTypeAddress.type.id = :typeId AND addressTypeAddress.address.id = :addressId",
	...
	public AddressTypeAddress getByCode(String code) {
		...
		try {
			TypedQuery<AddressTypeAddress> q = em
					.createQuery(
							"SELECT AddressTypeAddress FROM com.conx.logistics.mdm.domain.geolocation.AddressTypeAddress AS addressTypeAddress WHERE addressTypeAddress.code = :code",
							AddressTypeAddress.class);
	...
	public AddressTypeAddress getById(Long id) {
		...
		try {
			TypedQuery<AddressTypeAddress> q = em
					.createQuery(
							"SELECT AddressTypeAddress FROM com.conx.logistics.mdm.domain.geolocation.AddressTypeAddress AS addressTypeAddress WHERE addressTypeAddress.id = :id",
							AddressTypeAddress.class);
