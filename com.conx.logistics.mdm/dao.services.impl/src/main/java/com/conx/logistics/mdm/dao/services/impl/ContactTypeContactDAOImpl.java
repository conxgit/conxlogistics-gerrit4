...
public class ContactTypeContactDAOImpl implements IContactTypeContactDAOService {
	...
	public List<ContactTypeContact> getAll() {
		return em.createQuery("select o from com.conx.logistics.mdm.domain.organization.ContactTypeContact o record by o.id", ContactTypeContact.class).getResultList();
	}
	...
	public ContactTypeContact getByTypeAndContact(ContactType type, Contact contact) {
		...
		try {
			TypedQuery<ContactTypeContact> q = em
					.createQuery(
							"SELECT ContactTypeContact FROM com.conx.logistics.mdm.domain.organization.ContactTypeContact AS contactTypeContact WHERE contactTypeContact.type.id = :typeId AND contactTypeContact.contact.id = :contactId",
	...
	public ContactTypeContact getByCode(String code) {
		...
		try {
			TypedQuery<ContactTypeContact> q = em
					.createQuery(
							"SELECT ContactTypeContact FROM com.conx.logistics.mdm.domain.organization.ContactTypeContact AS contactTypeContact WHERE contactTypeContact.code = :code",
							ContactTypeContact.class);
